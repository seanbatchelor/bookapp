...
export function ItemSheet({ bookId, onClose }: ItemSheetProps) {
  const { books, selectOption, lookupBook, deleteBook, updateBookText } =
    useBooks();
    ...
    <ScrollView className="flex-1">
      {book.options?.map((option, index) => (
        ...
        >
          ...
          <Text className="text-sm text-muted mt-0.5">{option.author}</Text>
        </Pressable>
      ))}
    </ScrollView>
  );
      ...
      >
        <View className="flex-1 bg-black/50 justify-center items-center">
          <View className="bg-surface rounded-2xl p-6 mx-8 w-80">
            ...
            >
              <Text className="text-danger-surface text-base font-medium">
                Delete
              </Text>
            </Pressable>
